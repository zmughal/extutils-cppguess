...
sub new {
    ...
    if(!exists $self->{cc} || !defined $self->{cc}) {
        $self->{cc} = exists $self->{config}{cc} && defined $self->{config}{cc}
            ? $self->{config}{cc}
            : $Config::Config{cc};
    }
    ...
    if(!exists $self->{os} || !defined $self->{os}) {
        $self->{os} = exists $self->{config}{osname} && defined $self->{config}{osname}
            ? $self->{config}{osname}
            : $^O;
    }
...
sub _get_lflags {
    ...
    $self->guess_compiler || die; # TODO - Shouldn't we be expressing a reason?
...
sub _cc_is_msvc {
    ...
    return $self->{is_msvc}
        ||= ($self->_os =~ /MSWin32/ && File::Basename::basename($cc) =~ /^cl/i);
}
...
sub is_gcc {
    my $self = shift;
    $self->guess_compiler || die; # TODO - This should die with more information.
    return $self->{is_gcc};
}
...
C<ExtUtils::CppGuess> attempts to guess the system's C++ compiler that is
compatible with the C compiler that your perl was built with.

It can generate the necessary options to the L<Module::Build> constructor
or to L<ExtUtils::MakeMaker>'s C<WriteMakefile> function.

=head1 METHODS

=head2 new

Creates a new C<ExtUtils::CppGuess> object. Takes the path to the C compiler
as the C<cc> argument, but falls back to the value of C<$Config{cc}>, which
should be what you want anyway.

You can specify C<extra_compiler_flags> and C<extra_linker_flags> (as strings)
which will be merged in with the auto-detected ones.
...
If you specify the extra compiler or linker flags in the constructor, they'll
be merged into C<CCFLAGS> or C<OTHERLDFLAGS> respectively.
